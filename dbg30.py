#!/usr/bin/env python3
"""err 臂微调：iters × eta_inf × alpha（0/1 编码）找 err>=0.86 配置。"""
import sys
...
base = replace(CreditConfig(alpha=1.5, beta=1.0, energy_mode="full"),
               h1=32, h2=16)
print("err 臂：iters × eta × alpha")
for iters in (24, 32):
    for eta in (None, 0.07, 0.09):
        for alpha in (1.5, 1.8):
            cfg = replace(base, alpha=alpha)
            accs = [run(cfg, s, iters, eta) for s in range(3)]
            flag = " <== ALL>=0.80" if min(accs) >= 0.80 else ""
            tag = "adapt" if eta is None else f"{eta:.2f}"
            print(f"iters={iters} eta={tag} a={alpha:.1f} | "
