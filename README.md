...
- **阶段 H（承重墙决赛门，进行中）**：H0 基线复盘 + H1 字节 LM 探针（next-byte 预测，隐藏表征线性可读性随规模单调，对 BPTT 孪生记账）——**H1 首轮 + 全预算补档终判已跑（2026-09-09，4090，[results_phaseG/report_h1.md](results_phaseG/report_h1.md)）**：排除三伪影（加载崩溃档/探针欠拟合/尾档预算不满）后，全 5 档同预算 104 万步，**再拟合岭探针不随宽度单调且缓降（768→4032：0.280→0.280→0.275→0.257→0.238）**；iters 深推断消融（12→48）**否决"欠迭代"**；**cap 消融终判否决"cap 卡容量"**（4032@cap=8.0 重训于 step≈450k 谱发散复现 σmax→50.8）——**五个候选解释全部排除后换方案级变量：eta_w 宽度缩放消融已跑（2026-09-09）并方案级修复端到端不缩放**：µPC 缩放指数 0.5→1.0（eta_w2 ∝ 1/h）全预算重训 2832/4032，σmax 全程稳定（4.94/4.998）谱发散不复现，**端到端大档回落被修复并反超**（BPC 2832 3.993→**3.833**、4032 4.113→**3.848** 反超 1856 的 3.884，acc 严格单调 0.290/0.293）→ "不缩放"的端到端根因 = **宽度学习率缩放不足（方案级）非原理缺陷**；**但 H1 立式（线性可读性单调）仍否决**：eta1 探针 acc 2832=0.255/4032=0.252 仍 < 1856 的 0.275 → **BPC/探针解离以干净形态复现**，规模红利疑以非线性 k-WTA 稀疏集中承载——**下一步决定性实验：非线性/稀疏槽位探针**，详见 [docs/GPU_TASKS.md](docs/GPU_TASKS.md)+[docs/ROADMAP.md](docs/ROADMAP.md) §2
