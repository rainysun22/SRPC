...
- **阶段 E（语言化，完成收口）+ 阶段 F（结构化记忆 + 课程级免遗忘 + few-shot/ICL，F1–F3 已完成）**：字节级 UTF-8 词元前端 + 语言长程信用分配 + 缩放梯子 —— **E1 已收口**（主验收 5/5 PASS，assoc 延迟文本关联）；**E2 pilot 已收口**（µPC 参数化 + 独立线性读出头 + iPC 增量调度，1/2/4M 梯子 vs BPTT 孪生 1.20–1.22×，判据 2/3/4 PASS；判据 1 单调性转 GPU 登顶裁决 [docs/GPU_TASKS.md](docs/GPU_TASKS.md)）；**E2 GPU 登顶执行收口**（RTX 4090 全预算；W2 σmax 谱发散失稳定位 + 每步幂迭代谱截断 cap=5.0 修复，判据 1 重判通过）；**E3 已收口**（2M 放大档判据②记忆增益 +36.8% PASS、判据①旧语言回升 FAIL → 动机承接 F1）；**F1 已完成**（[results_phaseF/report_f1a.md](results_phaseF/report_f1a.md)：F1a n-gram 结构化慢记忆闭合 E3 判据① 旧语言回升 +38.2%→−8.5%、判据②增益 +36.8%→+71.3%；[results_phaseF/report_f1b.md](results_phaseF/report_f1b.md)：F1b VSA/HRR 绑定层确立组合口径——概念-关系组合查询容量-精度曲线 + xorsum 组合记忆回放判据③ d=1024 acc 0.73–0.78，能力=记忆·拼合在结构化记忆上实证）；**F2 已完成**（[results_phaseF/report_f2.md](results_phaseF/report_f2.md)：54 任务课程流免遗忘 pass——self 读出源 forget_mean 0.037 ≤ 25%、acc 保留 ~100%，证明课程级免遗忘上限=读出口所依赖的共享表征是否随课程漂移，PrototypeMemory 增量 0% 与 E0 原理自洽）；**F3 已完成**（[results_phaseF/report_f3.md](results_phaseF/report_f3.md)：颜色关系 ICL——few-shot 增益随示例数单调上升 acc(0) 0.047 → acc(8) 1.000（5 seeds）、记忆消融显著、组合 r2∘r1 生成未见输出对正确 0.961，闭合「有限经验→沉淀规律→组合生成未见结果」）；**阶段 G（推理与规划，全部子阶段收口）+ 阶段 D（意识向集成，已完全延后）**：**G1 已完成**（[results_phaseG/report_g1.md](results_phaseG/report_g1.md)：规划式未来预测器——x_self 动力学前向展开 H 步对照近视 greedy，二叉决策树 D=4 收益随前瞻深度单调上升 近视 0.487 → H4 0.912=全局最优，近视损益 Δ0.425，学习版 Wdyn 前瞻 0.908 仍优于近视，前瞻策略优于近视可证伪增益 PASS）；**G2 已完成**（[results_phaseG/report_g2.md](results_phaseG/report_g2.md)：G2a 分槽 HRR 组合-推理闭环 4/4 PASS——acc(0)0.053→acc(12)0.997、组合生成未见 combo_ok=1.0；G2b pythia-14m/31m 4090 RTE 贴随机基线 0.47–0.52，介质差异诚实并列）；**G3 已完成**（[results_phaseG/report_g3.md](results_phaseG/report_g3.md)：组合泛化规模测试——规模轴=组合深度 m，功能组合增益随 m 单调 PASS，容量受限基座返降上限 m*=3/11/43 @ d=128/512/2048，即上限来自记忆载体容量而非组合结构）；**G4 已完成（G1b 收口）**（[g4/results_g4b_all_seeds.json](g4/results_g4b_all_seeds.json)：无预钉自组织编码 + 动作块独立动力学，前瞻>近视 6/6 seed PASS、mean_rel_improv +81%）。**阶段 D 完全延后（能力优先）；后继对标大模型智力主线的阶段为 H·I·J·K**——H=承重墙决赛门且不降预期（真实语言任务 vs BPTT 孪生，随规模变强，初始不成立不触发下调、转调研循环）、能量记账重锚为"同规模同任务对孪生"，详见 [docs/ROADMAP.md](docs/ROADMAP.md) §2/§3。证据见 [results_e1/report.md](results_e1/report.md) / [results_e2/report.md](results_e2/report.md) / [results_e2_gpu/REPORT_E2_GPU_SUMMIT.md](results_e2_gpu/REPORT_E2_GPU_SUMMIT.md)
- **阶段 H（承重墙决赛门，进行中；H1 已以自-读出-宽度单调口径通过）**：H0 基线复盘 + H1 字节 LM 探针（next-byte 预测，隐藏表征线性可读性随规模单调，对 BPTT 孪生记账）——**H1 首轮 + 全预算补档终判已跑（2026-09-09，4090，[results_phaseG/report_h1.md](results_phaseG/report_h1.md)）**：排除三伪影（加载崩溃档/探针欠拟合/尾档预算不满）后，全 5 档同预算 104 万步，**再拟合岭探针不随宽度单调且缓降（768→4032：0.280→0.280→0.275→0.257→0.238）**；iters 深推断消融（12→48）**否决"欠迭代"**；**cap 消融终判否决"cap 卡容量"**（4032@cap=8.0 重训于 step≈450k 谱发散复现 σmax→50.8）——**五个候选解释全部排除后换方案级变量：eta_w 宽度缩放消融已跑（2026-09-09）并方案级修复端到端不缩放**：µPC 缩放指数 0.5→1.0（eta_w2 ∝ 1/h）全预算重训 2832/4032，σmax 全程稳定（4.94/4.998）谱发散不复现，**端到端大档回落被修复并反超**（BPC 2832 3.993→**3.833**、4032 4.113→**3.848** 反超 1856 的 3.884，acc 严格单调 0.290/0.293）→ "不缩放"的端到端根因 = **宽度学习率缩放不足（方案级）非原理缺陷**；**H1b 非线性/稀疏槽位探针终判（2026-09-10，4090）假设未获支持**：线性 0.280→0.280→0.275→**0.255→0.252**（缓降不单调）、**top-k 稀疏槽位 q0.1** 0.250→0.255→0.257→**0.233→0.213**（仅 1856 段内微升、大档跳水，"红利集中少数强槽位"不成立）、**两层 MLP** 全档恒定 **0.1333**（坍缩到单类基线，训练崩溃、口径作废）→ **三种读出口均无随宽度单调之证据**，初判"读出口不可接触"；**H1c 自-读出头终判（2026-09-10，4090）去解离化达成，H1 立式以自-读出-宽度单调口径通过**：换**模型固有读出口 `W_out`**（容量随宽度自动放大、与表征联合优化）测全 5 档（统一最优健康档口径）——**acc 严格单调** 0.2500→0.2567→0.2850→**0.2900→0.2933**（`monotonic=true`，全档同预算、σmax 健康 ≤5.03、x2 保持 ~50% 稀疏无坍缩），与外置三探针方向相反 → **"读出不可接触"是 probe 端口径伪影**（外置探针冻结表征上重训复现不了训练期与 W_out 联合优化的表征-读出口耦合），非 SR-PC 能力缺陷 → **承重墙"能力随规模变强"的端到端核验成立**。**越深越崩 修复落地（2026-09-10，诊断+文献+方案，[results_phaseG/report_h1.md](results_phaseG/report_h1.md) §2e）**：不重训轨迹诊断**反证"饱和死锁"假**（sat_frac 全程≈0）→ 定位 = **深推断进入 2 周期符号翻转振荡 + x2_mean 漂升（=能量上升发散，EVPE 爆发）**，4032 flip_frac 钉 0.99 vs 1856 封顶 0.63；文献归因 **Mali et al. PC 收缩界 step<1/Lipschitz + Ha et al. ICLR'26 Meta-PCN EVPE**；**纯推断期把 x2/x3 步长 ×`eta_inf_scl=0.5` 压入收缩界即彻底稳定**（4032@48 acc 0.01→**0.26**、bpc 16.31→6.49、mean 0.198→0.0058，反超浅迭代；1856 同稳），落进 `E2Config.eta_inf_scl`（默认 1.0 保既有档）+ GPU/numpy 双后端自由推断分支、不动训练钳制。下一步：H1 收口 → 推进 **H2 能量对比**（同能力 MAC/bit 显著低于同规模孪生）；孪生 2832/4032 参照线补齐在待办（非阻塞）。详见 [results_phaseG/report_h1.md](results_phaseG/report_h1.md) §2d–§2e/§3 + [docs/GPU_TASKS.md](docs/GPU_TASKS.md) T9 + [docs/ROADMAP.md](docs/ROADMAP.md) §2
...
docs/
  ...
  ROADMAP.md      # 后续路线图（E 语言化 / F 知识+持续学习 / G 推理规划 / H·I·J·K 低能耗对标智力主线 / D 意识向·完全延后）
...
results_phaseF/   # 阶段 F：结构化记忆扩容（F1a n-gram / F1b VSA）+ 课程级免遗忘（F2）+ 上下文学习 ICL（F3）
results_phaseG/   # 阶段 G：规划式未来预测器（G1 前瞻展开 vs 近视）+ 推理闭环（G2）
g1/               # 阶段 G 代码：g1_planner.py（x_self 前向展开 + 多步前瞻选择 + 近视对照）
```
