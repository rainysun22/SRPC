#!/usr/bin/env python3
"""Δ4 方差修复扫描 v3：alpha/beta 比例（类拉动强度）× iters（信用传播深度）。
eta_inf 按谱半径自动适配；eval=500 可靠口径。"""
import sys
...
def run(cfg, delay, train, eval_n, seed, mode, iters, orth=False):
    ...
    m.orth = orth
    m.eta_inf = min(0.3, 0.6 * 1.2 / lam)   # 0.6 安全系数
    m.iters = iters
...
print("=== alpha/beta sweep (iters=8) ===")
best = None
for alpha, beta in ((1.0, 1.0), (2.0, 1.0), (3.0, 1.0), (1.0, 0.5), (0.6, 0.1)):
    accs, dists = [], []
    for s in range(3):
        cfg = replace(base, alpha=alpha, beta=beta)
        a, dd = run(cfg, 4, train, eval_n, s, "error", 8)
        accs.append(a); dists.append(dd)
    mm = np.mean(accs)
    print(f"a={alpha:.1f} b={beta:.1f} | " + " ".join(f"{a:.3f}" for a in accs)
          + f" | mean={mm:.3f} distal={np.mean(dists):.3f}")
    if best is None or mm > best[0]:
        best = (mm, alpha, beta)

_, a0, b0 = best
print(f"\n=== iters sweep on a={a0:.1f} b={b0:.1f} ===")
for iters in (5, 8, 12, 20):
    accs = []
    for s in range(3):
        cfg = replace(base, alpha=a0, beta=b0)
        a, _ = run(cfg, 4, train, eval_n, s, "error", iters)
        accs.append(a)
