#!/usr/bin/env python3
"""0/1 编码 + a=1.5 在 T<=5 约束下（iters=3/5）能否达标。"""
import sys
...
base = replace(CreditConfig(alpha=1.5, beta=1.0, energy_mode="full"),
               h1=32, h2=16)
print("T<=5: 0/1 编码 + a=1.5")
for iters in (3, 4, 5):
    for eta in (None, 0.10, 0.15, 0.20):
        accs = [run(base, s, iters, eta) for s in range(3)]
        flag = " <== ALL>=0.80" if min(accs) >= 0.80 else ""
        tag = "adapt" if eta is None else f"{eta:.2f}"
        print(f"iters={iters} eta={tag:5s} | " + " ".join(f"{a:.3f}" for a in accs)
