#!/usr/bin/env python3
"""alpha_eval 细化 × 全判据（err/hebb/gap/distal）。"""
import sys
...
base = replace(CreditConfig(), h1=32, h2=16, energy_mode="full",
               alpha=1.0, beta=1.0)
for ae in (1.25, 1.5, 1.75):
    rows = []
    for s in range(3):
        ae_r, de = run(base, s, "error", 24, ae)
        ah, _ = run(base, s, "hebb", 24, ae)
        rows.append((ae_r, ah, ae_r - ah, de))
    m = np.mean(np.array(rows), axis=0)
    print(f"alpha_eval={ae:.2f} | "
