"""H2 有界验证 v9：完全判别式 PC（µPC/Meta-PCN 风格）对 free 能力爬升是否有效。

与前 DPC(0.275)/识别编码器(0.265) 区别：从前馈识别态非零启动沉降 + relu 门控
全 credit + 预测沉降目标(target propagation)把判别目标下沉到 W2/W1c。评估走
eval_recog（同一编码路径，无沉降/钳制）。孪生@768@300k≈0.42、@90k≈0.34。

用法（4090，/root/srpc_e2/srpc_src）：
    python -B scripts/h2_dpc2_validate.py 768 90000 0.0005 0.8 8
结果攒进 results_e2_gpu_eta/h2_dpc2_{h}_amp{a}_s{s}.json
"""
...
amp = float(sys.argv[4]) if len(sys.argv) > 4 else 0.8
settle = int(sys.argv[5]) if len(sys.argv) > 5 else 8
eval_every = int(sys.argv[6]) if len(sys.argv) > 6 else 15_000
smax_cap = float(sys.argv[7]) if len(sys.argv) > 7 else 5.0
...
cfg.recog_dpc2 = True
cfg.dpc2_amp = amp
cfg.dpc2_settle = settle
tag = f"amp{amp}_s{settle}_c{smax_cap}"

seed = 0
corpus = ByteCorpus(cfg)
full = len(corpus.train) - cfg.context - 1
steps = min(steps, full)
name = f"h2_dpc2_{h}_{tag}"
...
print(f"== {name}: {steps} steps, lr={recog_lr} amp={amp} settle={settle} "
      f"twin@end={twin_curve or 'NA'} ==", flush=True)

t0 = time.time()
for t in range(steps):
    x, y = corpus.train_window(t)
    m.train_step_dpc2(x, y)
    if (t + 1) % cfg.eval_every == 0 or t == steps - 1:
        ...
        with open(jpath, "w") as f:
            json.dump(dict(h=h, steps=steps, recog_lr=recog_lr, amp=amp,
                           settle=settle, curve=curve), f, indent=1, default=float)
