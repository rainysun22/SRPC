#!/usr/bin/env python3
"""多种子筛查：12 个种子在 iters=24 下 Δ=4 err 分布（自适应步长）。"""
import sys
...
cfg = replace(CreditConfig(), h1=32, h2=16, energy_mode="full",
              alpha=1.0, beta=1.0)
print("12 seeds @ iters=24, eta=0.06")
accs = [run(cfg, s, 24, 0.06) for s in range(12)]
for s, a in enumerate(accs):
    print(f"  seed{s}: {a:.3f} {'PASS' if a >= 0.80 else 'FAIL'}")
print(f"mean={np.mean(accs):.3f} pass={sum(a >= 0.80 for a in accs)}/12")
