#!/usr/bin/env python3
"""完整验证：0/1 编码 + a=1.5/b=1.0/iters=24，全判据 + 分段稳定性。"""
import sys
...
def run(cfg, delay, train, eval_n, seed, mode, iters, lo, hi):
    rng = np.random.default_rng(seed * 3000 + 7)
    X, y = make_seq(cfg, rng, train + eval_n, delay, lo, hi)
...
cfg = replace(CreditConfig(alpha=1.5, beta=1.0, energy_mode="full"),
              h1=32, h2=16)
iters, lo, hi = 24, 0.0, 1.0
print("per-seed（Δ=4: train 10000 eval 1500；Δ=1: train 5000 eval 1500；bit=0/1）")
rows = []
for s in range(3):
    a4e, d4e = run(cfg, 4, 10000, 1500, s, "error", iters, lo, hi)
    a4h, _ = run(cfg, 4, 10000, 1500, s, "hebb", iters, lo, hi)
    a1e, d1e = run(cfg, 1, 5000, 1500, s, "error", iters, lo, hi)
    a1h, _ = run(cfg, 1, 5000, 1500, s, "hebb", iters, lo, hi)
